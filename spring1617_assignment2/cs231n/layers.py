...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        x_normalized = x_centre / np.sqrt(batch_var + eps)  # (N, D)
        ...
        # cache = x_normalized, batch_var, x_centre, eps, gamma
...
def batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    # x_normalized, batch_var, x_centre, eps, gamma = cache
    x_normalized = cache.get('x_normalized')
    batch_var = cache.get('batch_var')
    x_centre = cache.get('x_centre')
    eps = cache.get('eps')
    ...
    dgamma = (np.ones([1, N]).dot(dout * x_normalized)).flatten()   # (D,)
    dx_normalized = dout * gamma   # (N, D)
    dbatch_var = (np.ones([1, N]).dot(dx_normalized * x_centre) *
                  (- 1.0 / 2.0) * (batch_var + eps) ** (-3.0 / 2.0)).flatten()   # (D,)
    dx_centre = dx_normalized * (1.0 / np.sqrt(batch_var + eps))   # (N, D)
...
def batchnorm_backward_alt(dout, cache):
    ...
    dgamma = (np.ones([1, N]).dot(dout * x_normalized)).flatten()   # (D,)
    dx_normalized = dout * gamma   # (N, D)
    dx = dx_normalized * (1 - 1 / N) * (1 / np.sqrt(batch_var + eps)) * \
         (1 - (1 / (batch_var + eps)) * (x_centre ** 2) * (1 / N))   # (N, D)
    # pass
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
        ...
        for c in range(C):
            for v_idx in range(H_out):
                for h_idx in range(W_out):
                    ...
                    dx[n, c, x_vert:x_vert + pool_h, x_horz:x_horz + pool_w] += dout[n, c, v_idx, h_idx] * (
                    x_sub == np.amax(x_sub))
    # pass
    ###########################################################################
    #                             END OF YOUR CODE                            #
